...
const exercise_details = async (req, res) => {
  try {
    ...
    if (!result) return res.status(404).json("Exercise not founded");
    return res.status(200).json(result);
  } catch (error) {
    console.error(error);
...
const exercise_create = async (req, res) => {
  try {
    ...
    if (error.message === "Duplicate Exercise")
      return res.status(400).json("Duplicate Exercise");
...
const exercise_multi_create = async (req, res) => {
  try {
    ...
    if (!result)
      return res
        .status(404)
        .json("Exercise already exists in database. Check log to know which");
    return res.status(201).json({ IDs: result.insertId, Exercise: req.body });
  } catch (error) {
    if (error.message === "Exercise duplicated")
      return res.status(400).json("Exercise already saved in database");
    return res.status(500).json("Internal server error");
  }
};
...
const exercise_update = async (req, res) => {
  try {
    ...
    if (!exercise) return res.status(404).json("Exercise not founded");
...
const exercise_delete = async (req, res) => {
  try {
    ...
    if (!deletedExercise) return res.status(404).json("Exercise not founded");
